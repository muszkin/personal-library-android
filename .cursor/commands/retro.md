...
**Usage:** Use slash command `/retro` in chat to activate retrospective workflow at end of session.
...
- **Quality Filter (A lesson is durable ONLY if it is):**
  - ✅ **Universal & Reusable:** Pattern that applies to many future tasks across different projects.
  - ✅ **Abstracted:** General principle (e.g., "Always verify environment variable exists before use"), not tied to specific session details.
...
- **CRITICAL:** Do NOT auto-modify global.mdc directly. Use evolution log + integration protocol.
...
- Human manually applies to global.mdc
- AI verifies correctness

### **Why Not Fully Automatic?**
- **global.mdc Complexity:** 575+ lines, 51 sections, complex structure
...
1. **Read** target rule file to understand structure (global.mdc)
...
- **Doctrine File Selection Protocol:** Prioritize project-level rules (`.cursor/rules/`), fallback to global rules if lesson is truly universal. Update global.mdc directly with operational lessons. This file (`retro.md`) contains retrospective process itself, not operational lessons.
...
- Depth Requirement Recognition: "più a fondo possibile" means systematic analysis, not approximations
...
- Status Communication: Use clear status markers (✅ completed, ⚠️ corrected, 🚧 in progress)
...
- **Report Structure:**
  1. **Evolution Log Entry:**
     - State which lesson(s) were saved to `.cursor/doctrine-evolution/log.md`
